...
echo "Successfully installed Workbox CLI!"
#####################
# END MODIFIED CODE #
#####################
...
if [[ ! -x $(type -P workbox >/dev/null) ]] && [[ ! -x $(command -v workbox) ]]; then
  echo "Could not find the Workbox CLI! Please see the log above for more info."
  exit 1
else
  workbox generateSW
  echo "Successfully generated service worker!"
fi
#####################
# END MODIFIED CODE #
#####################
